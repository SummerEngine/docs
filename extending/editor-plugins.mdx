...
Measured on **Summer 0.5.55**, the current shipped build: throwaway headless editors took
ports 6550, 6551 and 6552 across successive runs and rewrote both files while the user's
real editor was running. On exit, the port file points at a dead port, so nothing can
find the real editor any more. A later build may suppress this for batch-mode instances;
assume it does not unless your version's [changelog](/changelog/overview) says so.
