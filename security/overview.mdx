...
description: "How Summer Engine protects your code and data"
icon: "shield"
---
...
We use additional model providers as part of routing on Auto and MAX. Privacy Mode applies to all model providers we route to. See [Privacy Mode](/security/privacy-mode) for details.

### Data Storage and Processing

**Turbopuffer** - *Stores code data*  
Powers project intelligence and code search. We store embeddings of your indexed project alongside the source chunks they were computed from, the file path within your project, and file metadata (size, modification time, content hash). The stored source text is indexed for full-text search. This data lives with Turbopuffer on Google Cloud servers in the US. You can read more on [Turbopuffer's security page](https://turbopuffer.com/docs/security).
...
**PostHog** - *Product analytics, not code storage*  
Usage analytics, hosted on PostHog's EU infrastructure. We record events such as "AI request made" or "project opened" together with properties describing the event. We do not send your source files here, and browser autocapture is disabled by default.
