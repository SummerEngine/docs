...
Summer uses **per-project indexing** with **intelligent caching**—similar to how Cursor indexes a repo. Each project has its own isolated search index. When you open a project, Summer scans and indexes your codebase, then uses RAG (Retrieval-Augmented Generation) to find relevant context when you ask questions.

**Smart caching** means Summer remembers what it learned between sessions *within that project*. The index persists so you don't start from scratch every time you open Summer.
