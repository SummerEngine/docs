---
title: "How do I make a game with just AI?"
description: "AI game dev: build a complete game with full AI assistance, no coding required. Vibe coding for game development."
icon: "sparkles"
---

## Make a Game with Full AI Assistance

Summer lets you build complete games by describing what you want. No coding experience required. This is AI game development—describe your vision, and Summer writes the code, creates assets, and sets up your game.

## What is AI Game Dev?

**AI game dev** (sometimes called "vibe coding" for games) means you describe your game in plain English and the AI does the technical work. You focus on the creative vision; Summer handles the implementation.

## Step 1: Get Summer

<Steps>
  <Step title="Download Summer">
    Visit [summerengine.com](https://summerengine.com) and download for Mac or Windows
  </Step>
  <Step title="Create your account">
    Sign in with Google, GitHub, or email. No credit card required
  </Step>
</Steps>

## Step 2: Describe Your Game

Create a new project, then tell Summer what you want. Examples:

- **"Create a 2D platformer with a character that can jump and double-jump"**
- **"Make a top-down shooter with enemies that chase the player"**
- **"Build a puzzle game where you match colored blocks"**
- **"Add a main menu with Start and Quit buttons"**
- **"Generate a low-poly tree for my forest scene"**

Summer understands natural language. You don't need to be precise—just describe what you want.

## Step 3: Review and Iterate

- **Review changes** — Summer shows you what it will change before applying
- **Undo anytime** — All AI changes use the standard undo system
- **Refine with follow-ups** — "Make the jump higher" or "Add more platforms"

## What You Can Build with AI Alone

- **Game logic** — Movement, combat, scoring, menus
- **2D and 3D assets** — Characters, environments, props (AI-generated)
- **Audio** — Music and sound effects (AI-generated)
- **UI** — Menus, HUDs, buttons
- **Physics** — Collision, gravity, interactions

## Next Steps

<CardGroup cols={2}>
  <Card title="First Chat Tutorial" icon="message-square" href="/essentials/first-chat">
    Learn effective prompting for game development
  </Card>
  <Card title="AI Asset Generation" icon="palette" href="/art-system/ai-generation">
    Create 2D art, 3D models, and audio with AI
  </Card>
  <Card title="AI Operations" icon="brain" href="/ai-tools/operations">
    What Summer can do in your project
  </Card>
  <Card title="Publishing" icon="upload" href="/publishing/overview">
    Export and release your game
  </Card>
</CardGroup>

---

Need help or have questions? Reach out to our founders at [founders@summerengine.com](mailto:founders@summerengine.com) or join our community on [Discord](https://discord.gg/yUpgtxnZky) for fast responses.
