...
and the automated runtime sandbox. Multiplayer sessions in the current runtime run
host-authoritatively over ENet; the `SummerMultiplayerPeer` transport (stock `@rpc` /
`MultiplayerSynchronizer` compatibility) is a published contract still being built.
