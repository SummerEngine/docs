...
<Info>
  GodotSteam is a native, version-sensitive integration. Do not assume identical behavior or
  that every upstream tutorial applies. Match the plugin build to Summer Engine's current
  upstream base, test it in a committed copy, then verify achievements, overlay, input,
  cloud saves, and exports on every target platform. See
  [GDExtension compatibility](/extending/gdextension).
</Info>
