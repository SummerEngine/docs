...
In Summer 0.5.55 and earlier, adding `--editor` starts Summer's local API server, which
**overwrites `~/.summer/api-port` and regenerates `~/.summer/api-token`**. That is how the
CLI and your agent find a running editor. A headless editor takes the port for itself,
rotates the token underneath whatever was already running, and on exit leaves the file
pointing at a dead port.

Do not run `--headless --editor` against a project the user has open, and expect to have to
restart the real editor afterwards if you run it at all.

This is fixed in later builds, which no longer publish the machine-global binding from
batch-mode instances. If you are unsure which you have, assume the older behaviour.
