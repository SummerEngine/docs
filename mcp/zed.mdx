---
title: "Build Games in Zed with Summer Engine MCP"
description: "Connect Summer Engine's MCP server to Zed and build games with AI inside the fast, AI-powered editor."
icon: "zap"
---
...
<Tip>
**Fastest path.** `npx -y summer-engine@latest setup zed --yes` writes the config below into `~/.config/zed/settings.json`, installs the skill library to `~/.agents/skills`, and runs `summer doctor`. The steps that follow are the manual version of the same thing.
</Tip>
