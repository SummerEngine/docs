...
description: "All 44 Summer Engine MCP tools: scene editing, debugging, project context, asset library search, and AI generation of images, 3D models, audio, video, and motion."
icon: "wrench"
---

## Overview

The Summer Engine MCP server exposes 44 tools across five categories: **Scene** (14), **Debug and Play** (9), **Project** (9), **Asset Library** (6), and **Generation** (6).

Scene, Debug, and Project tools talk to the running engine on `localhost:6550` and require Summer Engine to be open with a project loaded. If the engine is not running, those tools return a clear error and a `summer run` hint. Asset Library and Generation tools call the Summer Engine cloud and require you to be signed in (run `npx summer-engine login` once); the import side of those tools needs the engine running too.
