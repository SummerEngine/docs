...
description: "Summer Engine MCP tools for scene editing, project files, debugging, visual verification, assets, generation, Summer Cloud, and creator workflows."
icon: "wrench"
---

{/* This checked-in reference is synchronized with the canonical 2.8 MCP definitions. `npm run check:mcp-registry` rejects missing, duplicate, or extra tool headings. */}

## Overview

The Summer Engine 2.8 MCP candidate registers 62 tools for **Scene**, **Debug and Play**, **Visual Verification**, **Project**, **Project Files**, **Asset Library**, **Generation**, **Summer Cloud**, and **Creator** workflows. Published 2.7.0 registers 58; the four additions are the Creator tools documented below. Client onboarding and editor routing do not add separate tools.
...
Every scene mutation names its exact target with `scenePath`, such as `res://main.tscn`. The target does not need to be the active editor tab.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenePath` | string | Yes | Exact target scene, e.g. `res://main.tscn` |
...
**Example:** `summer_add_node(scenePath="res://main.tscn", parent="./", type="DirectionalLight3D", name="Sun")`
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenePath` | string | Yes | Exact target scene |
...
**Example:** `summer_set_resource_property(scenePath="res://main.tscn", nodePath="./Player/CollisionShape3D", resourceProperty="shape", subProperty="size", value="Vector3(1, 2, 1)")`
...
Save an explicit scene to disk. Dedicated mutation tools already append one final save; use this tool for a standalone save or save-as.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenePath` | string | Yes | Scene to save, e.g. `res://main.tscn` |
| `path` | string | No | Optional save-as destination, e.g. `res://levels/level2.tscn` |

---

### summer_open_scene

Open a scene file as the visible editor tab. This is navigation only; scene mutation tools use their own `scenePath`. Prefer `summer_get_project_context` and `summer_open_main_scene` over guessing paths.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenePath` | string | Yes | Exact scene that receives the instance |
...
Create a support-ready Markdown report containing engine health, diagnostics, filtered console output, debugger errors and warnings, and an agent handoff prompt. Authentication tokens and project file contents are omitted, but users should review local paths and stack traces before sharing it.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `issue` | string | No | User-visible issue or reproduction summary |
| `output_path` | string | No | Markdown destination; defaults to the project root |
| `include_play_session` | boolean | No | Briefly launch the game and collect post-play diagnostics |
| `play_wait_ms` | number | No | Wait after launch when collecting a play session |
| `max_console_lines` | number | No | Maximum filtered console lines |
| `max_debugger_entries` | number | No | Maximum debugger errors and warnings |
| `include_doctor` | boolean | No | Include `summer doctor` checks (default `true`) |
...
Capture an engine frame and return the image directly to the MCP client.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `target` | enum | No | `viewport` (default), `scene`, or `game` |
| `scenePath` | string | No | Exact scene for an offscreen `scene` capture |
| `framing` | enum | No | `auto`, `top`, `front`, or `iso` for scene capture |
| `size` | number[] | No | Scene capture size as `[width, height]` |
| `nodePath` | string | No | Node to frame during scene capture |

`viewport` captures the visible editor view. `scene` renders an exact scene file offscreen without changing the visible tab. `game` captures the currently running game and requires the desktop bridge.
...
Get a scene tree. Pass `scenePath` to inspect that exact loaded scene; omit it only when you intentionally want the visible editor tab.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenePath` | string | No | Exact `res://` scene to inspect |
...
These tools operate inside the engine-bound project and return exact file receipts. They do not use a broad project writer lock. If another agent or the editor changed a file after it was read, the stale overwrite is refused; reread the file and decide whether the edit still applies.

### summer_read_file

Read a UTF-8 project file and return its complete contents plus a full-file SHA-256 receipt.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `path` | string | Yes | Project path beginning with `res://` |
| `max_bytes` | number | No | Maximum bytes to return (default 200,000) |

---

### summer_write_file

Create or safely overwrite a complete text file. Exactly one write guard is required.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `path` | string | Yes | Project path beginning with `res://` |
| `content` | string | Yes | Complete new UTF-8 content |
| `create_only` | boolean | Conditional | Use `true` for a new file; refuses if it already exists |
| `expected_sha256` | string | Conditional | Use the receipt from `summer_read_file` when overwriting |

---

### summer_replace_text

Replace exact text in an existing project file. The tool reads the current file, requires a unique match by default, and submits a SHA-256-guarded write.
...
| `replace_all` | boolean | No | Replace every exact occurrence instead of requiring one unique match |
...
| `scenePath` | string | Conditional | Required with `parent`; exact scene that receives the 3D model |
...
AI generation runs in Summer Engine Studio and consumes credits. Workflow discovery is read-only, and asset-sheet slicing processes an existing Summer asset. These tools need you signed in (`npx summer-engine login`). Confirm with the user before spending on paid generation, then import results into the project with `summer_import_asset_by_id` or `summer_import_from_url`.

### summer_get_studio_workflow

List Summer Engine Studio's Guided workflow recipes, or retrieve one exact
recipe with its starter prompt, ordered steps, required MCP tools, support
level, and honest limitations. This read-only discovery tool does not spend
credits.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `workflowId` | string | No | Guided workflow ID, for example `sprite-sheet`, `character-pack`, `asset-pack`, or `component-compiler`. Omit to list all workflows. |
...
Detect and crop every distinct game asset from an existing Summer image asset
sheet. The workflow can remove the background, detect named or category-aware
bounding boxes, upload the slices, and return their URLs, names, categories,
bounds, and UI widget metadata.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `assetId` | string | Yes | Summer image asset ID, usually returned by `summer_generate_image` |
...
These four tools are the only MCP registrations added between published 2.7.0
and 2.8. Creator publishing uses the versioned Summercraft API and requires
explicit confirmation of the exact artifact and target. Configuration is
local and non-secret; tokens are never accepted or returned.

### summer_creator_publish

Prepare and publish one exact exported Summer `.pck`. First call with
`confirm=false`, show the returned project, version, digest, size, artifact,
channel, and notes to the user, then retry with `confirm=true` only after the
user approves that exact target.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `artifact` | string | Yes | Exact path to the exported `.pck` |
| `version` | string | Yes | Immutable release version |
| `project` | string | No | Project root; defaults to the current working directory |
| `manifest` | string | No | Optional JSON release manifest path |
| `projectId` | string | No | Creator project ID; defaults to shared Summer configuration |
| `channel` | string | No | Release channel; defaults to configured channel or `production` |
| `notes` | string | No | Release notes included in confirmation and the local audit |
| `confirm` | boolean | No | Must be `true` only after exact user approval (default `false`) |

---

### summer_creator_releases

List real creator-owned releases from the versioned creator API.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `projectId` | string | No | Creator project ID; defaults to shared Summer configuration |
| `limit` | number | No | Page size from 1 to 100 (default 20) |
...
Request runtime logs for a creator project or release. This tool fails closed
with the exact recovery action when the required platform route or token scope
is unavailable; it does not invent placeholder logs.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `projectId` | string | No | Creator project ID; defaults to shared Summer configuration |
| `releaseId` | string | No | Optional release ID |
| `limit` | number | No | Maximum rows from 1 to 1000 (default 100) |

---

### summer_creator_config

Read or update shared non-secret Summer configuration. Mutations require
`confirm=true`; tokens are never accepted or returned.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `action` | enum | Yes | `list`, `get`, `set`, or `unset` |
| `key` | enum | Conditional | Required for `get`, `set`, and `unset` |
| `value` | string | Conditional | Required for `set` |
| `confirm` | boolean | No | Required `true` for `set` and `unset` (default `false`) |
