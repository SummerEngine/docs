---
title: "Build Games in Cursor with Summer Engine MCP"
description: "Connect Summer Engine's MCP server to Cursor in under a minute and build games with AI inside your editor. One-click or manual configuration."
icon: "mouse-pointer"
---
...
<Tip>
**Fastest path.** `npx -y summer-engine@latest setup cursor --yes` writes the config below into `~/.cursor/mcp.json`, installs the skill library to `~/.agents/skills (Cursor reads the agentskills.io folder)`, and runs `summer doctor`. The steps that follow are the manual version of the same thing.
</Tip>
...
<Card
  ...
  icon="plug"
  href="cursor://anysphere.cursor-deeplink/mcp/install?name=summer-engine&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsInN1bW1lci1lbmdpbmVAbGF0ZXN0IiwibWNwIl19"
>
...
3. You should see **summer-engine** listed, with its tools available
