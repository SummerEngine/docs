---
title: "Build Summer Games in VS Code"
description: "Connect Summer Engine's MCP server to VS Code or GitHub Copilot and build a Summer game from Microsoft's editor and Copilot's AI agent."
icon: "square-code"
---
...
## Summer Engine in VS Code and GitHub Copilot
...
<Tip>
**Fastest path.** `npx -y summer-engine@latest setup vscode-copilot --yes` writes the config below into `VS Code's user mcp.json`, installs the skill library to `~/.agents/skills`, and runs `summer doctor`. The steps that follow are the manual version of the same thing.
</Tip>
...
2. You should see **summer-engine** listed, with its tools available
